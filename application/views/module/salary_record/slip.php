  ...
  <div class="max-w-3xl mx-auto">
    ...
    <div class="slip bg-white rounded-2xl shadow p-6" id="printArea">
      ...
      <div class="flex items-center justify-between border-b pb-4 mb-4">
        <div>
          <h1 class="text-2xl font-semibold"><?= $cmp['company_name'] ?></h1>
          <p class="text-sm text-gray-600"><?= $cmp['address'] ?></p>
        </div>
        <div class="text-right">
          <p class="text-sm text-gray-500">Periode: <span class="font-medium"><?= $periode ?></span></p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 text-sm">
        <div>
          ...
          <p class="font-medium"><?= $emp['nama_pegawai'] ?></p>
        </div>
        <div>
          <p class="text-gray-500">Jabatan</p>
          <p class="font-medium"><?= $pst['name'] ?></p>
        </div>
        <div>
          <p class="text-gray-500">NIK</p>
          <p class="font-medium"><?= $emp['nik'] ?></p>
        </div>
      </div>
          ...
          <table class="w-full text-sm">
            <tbody>
              <tr class="border-t">
                ...
                <td class="py-2 text-right font-medium"><?= number_format($emp['income'], 2) ?></td>
              </tr>

              <?php foreach ($emp['plus'] as $i): ?>
                <tr class="border-t">
        ...
        <div class="md:flex md:justify-end">
          <div class="w-full md:w-1/2">
            ...
            <div class="flex justify-between py-1">
              ...
              <div class="font-medium"><?= number_format($emp['totalMinus'],2) ?></div>
            </div>
            <div class="flex justify-between pt-3 border-t mt-3">
              <div class="text-base font-semibold">Take Home Pay</div>
              <div class="text-base font-semibold"><?= number_format($emp['thp'],2) ?></div>
            </div>
          </div>
        </div>
      </div>
