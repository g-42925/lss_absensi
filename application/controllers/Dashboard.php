...
class Dashboard extends CI_Controller {
    ...
    public function index() {
                ...
                $izin2 = $this->db->query("SELECT * FROM tx_request_izin WHERE (tipe_request='i' OR tipe_request='c'  OR tipe_request='csh' OR tipe_request='cth') And is_status=1 AND tanggal_request_end>='$today' and company_id = ?",[$companyId])->num_rows();
                ...
                $data['a_tl'] = $this->db->query("select * from assignment a join m_pegawai mp on mp.pegawai_id = a.employee_id where mp.company_id = ? and until >= ?",[$companyId,$today])->num_rows();

                $ts = $this->db->query("SELECT * FROM tx_absensi WHERE (is_status='ts' OR is_status='th') AND tanggal_absen='$today' and company_id = ?",[$companyId])->num_rows();
